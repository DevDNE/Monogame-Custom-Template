...
public class TitleState : GameState
{
  ...
  public TitleState(
    ServiceProvider serviceProvider, SpriteFont font,
  ...
  public override void Draw(SpriteBatch spriteBatch, GameTime gameTime)
  {
    ...
    const string title = "Shooter";
    Vector2 titleSize = _font.MeasureString(title);
    spriteBatch.DrawString(_font, title, new Vector2(_viewportWidth / 2f - titleSize.X / 2f, 120), Color.White);
    const string subtitle = "Twin-stick arena survival";
    Vector2 subSize = _font.MeasureString(subtitle);
    spriteBatch.DrawString(_font, subtitle, new Vector2(_viewportWidth / 2f - subSize.X / 2f, 156), new Color(170, 180, 210));
    const string hint = "WASD move   Mouse aim   Click / Space fire";
    Vector2 hintSize = _font.MeasureString(hint);
    spriteBatch.DrawString(_font, hint, new Vector2(_viewportWidth / 2f - hintSize.X / 2f, _viewportHeight - 60), new Color(180, 180, 200));
