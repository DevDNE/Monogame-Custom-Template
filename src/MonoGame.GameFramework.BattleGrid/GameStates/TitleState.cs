...
public class TitleState : GameState
{
  ...
  {
    // UIManager.Update handles hover + click dispatch.
  }
  ...
  public override void Draw(SpriteBatch spriteBatch, GameTime gameTime)
  {
    ...
    const string title = "BattleGrid";
    Vector2 titleSize = _font.MeasureString(title);
    spriteBatch.DrawString(_font, title, new Vector2(_viewportWidth / 2f - titleSize.X / 2f, 120), Color.White);

    const string subtitle = "Grid-based dueling";
    Vector2 subtitleSize = _font.MeasureString(subtitle);
    spriteBatch.DrawString(_font, subtitle, new Vector2(_viewportWidth / 2f - subtitleSize.X / 2f, 156), new Color(170, 180, 210));
